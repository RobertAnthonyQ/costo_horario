...
// Mock data - mismos datos pero mejor organizados
const machines = [
  {
    id: 1,
    codigo: "EQ-001",
...
export default function MaquinasPage() {
  ...
  const activeMachines = machines.filter((m) => m.estado === "Activo").length;
  const maintenanceMachines = machines.filter(
    (m) => m.estado === "Mantenimiento"
  ).length;
  const totalValue = machines.reduce((sum, m) => sum + (m.valor || 0), 0);
          ...
          <Table>
            ...
            <TableBody>
              {filteredMachines.map((machine) => (
                <TableRow key={machine.id} className="hover:bg-muted">
